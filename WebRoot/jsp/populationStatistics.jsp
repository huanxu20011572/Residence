...
//							});  
						    ...
						    jsonObj.xData=xData;
						    jsonObj.yData=yData;
						     if(jsonObj.xData.length==0){
        		alert("流动人口和常住人口暂无数据信息!");
        		return;
        	}							
							loadChart(jsonObj);
						}
	...
	function loadChart(json){
        ...
        function (ec) {
        	...
        	for(var i=0;i<json.xData.length;i++){
        		xData[i]=json.xData[i];
        		yData[i]=json.yData[i];
        		
        	}
        	if(json.xData.length==2){
        		ms.push({value:json.yData[0], name:json.xData[0], itemStyle:{
                              normal:{color:'#00E3E3'}
                          }});
                ms.push({value:json.yData[1], name:json.xData[1],   itemStyle:{
                              normal:{color:'#46A3FF'}
                          }});
        	}
        	else{
