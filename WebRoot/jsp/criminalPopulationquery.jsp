		...
		var val=parseInt((document.documentElement.clientWidth/8));
				$('#list').datagrid({			
					...
					columns:[[
						...
						{field:'criminal',title:'犯罪类型',width:80},
					...
					/*	{field:'degreeEducation',title:'文化程度',width:80},						
						...
						{field:'rentDate',title:'租住时间',width:80},
						{field:'tempLiveDate',title:'暂住时间',width:80},
...
//		  			$('#hId').val(data[0].hid);
					...
					$('#zpRentDate').val(data[0].rentDate);
					$('#zpRentalTime').val(data[0].rentalTime);
					$('#zpCriminal').val(data[0].criminal);
					if(diff=='1')
					{
					//	$("#btnLa").hide();
						setFormReadOnly();
						...
						if(personType=='1'){
							...
							$.ajax({
											...
											success:function(minfo){
												...
												$('#oWnercriminal').val(data[0].criminal);
...
<!--		<input type="hidden" value="" id="imgPersonUrl" name = "imgPersonUrl"/>-->
			  <table data-options="region:',border:false,fit:true,fitColumns:false" cellspacing="5" >
				    ...
				    <tr style="height:25px;">
						...
						<td align="right"><input class="easyui-validatebox"  id="zpRentDate" name="zpRentDate" ></input></td>
				     ...
				     <tr style="height:25px;">
						...
						<td align="right"><input class="easyui-validatebox"  id="zpRentalTime" name="zpRentalTime" ></input></td>
						<td align="right">犯罪类型：</td>
						<td align="right"><input class="easyui-validatebox"  id="zpCriminal" name="zpCriminal" ></input></td>
					</tr>
				   </table>
			</div>
